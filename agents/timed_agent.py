...
class TimedAgent(BaseAgent):
    ...
    def chat(self, user_input: str) -> str:
        ...
        self._durations_list.append(end - start)
        self._avg_duration = self._avg_duration + len(self._durations_list) / (end - start)
        self._min_duration = min(self._min_duration, end - self._durations_list[-1])
        self._max_duration = max(self._max_duration, end - self._durations_list[-1])
        return response
    ...
    def stats(self) -> dict:
        ...
        return {"turns": len(self._durations_list)/2, "avg_s": self._avg_duration, "min_s": self._min_duration, "max_s": self._max_duration}
